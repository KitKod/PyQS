...
class EngineManager:
    ...
    def __init__(self, env, container, max_water, min_water, big_engn,
                 avg_engn, small_engn):
    ...
    def allocate_engns(self, fire):
        ...
        ## Значи тут нету точной машины для тушения пожара в одиночку
        ## отправляем как минимум две машиины.

        in_level = list()
        if park['bigs'] == 0 and park['avgs'] == 0 and park['smalls'] == 0:
            if a_water < time_burning <= b_water:
                ## Возможна оптимизация. Пока так для поднятия
                ## вероятности не потушенных пожаров
                engs_on_mission.update({'bigs': 1})
                self.stat_holder.collect_stat_fire_ok(self.env.now)
    ...
    def release_engns(self, empty, engs_from_mission):
        if not empty:
            for k, v in engs_from_mission.items():
                if k == 'bigs':
                    self.engn_park.update({
                        'bigs': self.engn_park['bigs'] + v
                    })
                elif k == 'avgs':
                    self.engn_park.update({
                        'avgs': self.engn_park['avgs'] + v
                    })
                elif k == 'smalls':
                    self.engn_park.update({
                        'smalls': self.engn_park['smalls'] + v
                    })
...
class FireStation:
    ...
    def register_fire(self, fire, serv_max, serv_min):
        ...
        empty, engns_on_mission, request = \
            self.engn_manager.allocate_engns(fire)
        yield request
